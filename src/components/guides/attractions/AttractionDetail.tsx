...
export default function AttractionPage({ data, onAsk, onNavigate, onBack, layoutId, scrollRef }: { data: any; onAsk?: () => void; onNavigate?: () => void; onBack?: () => void; layoutId?: string; scrollRef?: React.RefObject<HTMLDivElement | null> }) {
  ...
  return (
    <div className="at-page" style={{ fontFamily: "var(--font-dm-sans-global), 'DM Sans', -apple-system, BlinkMacSystemFont, sans-serif", background: '#fff', color: '#222', lineHeight: 1.5, maxWidth: 430, margin: '0 auto', overflowX: 'hidden', paddingBottom: 80, WebkitFontSmoothing: 'antialiased' }}>
      ...
      {/* ═══ NAV ═══ */}
      <nav style={{ position: 'fixed', top: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 430, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '28px 16px 10px', zIndex: 100, transition: 'background .3s, box-shadow .3s', ...(navScrolled ? { background: 'rgba(255,255,255,.97)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', boxShadow: '0 1px 0 rgba(0,0,0,.06)' } : {}) }}>
