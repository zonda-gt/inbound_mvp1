...
export function detectLanguage(text: string): string {
  ...
  if (cjkCount / text.length > 0.3) {
    // If more than 30% CJK characters, it's likely Chinese/Japanese/Korean
    // Simple heuristic: check for hiragana/katakana for Japanese
    if (/[\u3040-\u309f\u30a0-\u30ff]/.test(text)) return "ja";
    // Check for Hangul for Korean
